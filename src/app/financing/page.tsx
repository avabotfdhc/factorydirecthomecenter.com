    ...
    <>
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
          <div className="max-w-3xl">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-xs font-bold tracking-[0.3em] uppercase text-[var(--color-teal-light)]">Loans &amp; Lenders</span>
            </div>
            <h1 className="font-serif text-5xl lg:text-6xl xl:text-7xl font-light tracking-tight mb-6">
              Financing That <span className="italic text-[var(--color-teal-light)]">Works</span><br />for You
            </h1>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="max-w-2xl mb-16">
            ...
            <h2 className="font-serif text-4xl lg:text-5xl font-light tracking-tight mb-6">
              Loan <span className="italic text-[var(--color-teal)]">Options</span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loanTypes.map((loan) => (
              <div key={loan.title} className="border border-[var(--color-charcoal)]/8 bg-white hover:border-[var(--color-teal)]/30 transition-all duration-500 flex flex-col">
                <div className="p-8 border-b border-[var(--color-charcoal)]/5">
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    <span className="inline-block bg-[var(--color-teal)]/10 text-[var(--color-teal)] text-xs font-bold tracking-wider uppercase px-3 py-1">{loan.downPayment}</span>
                  </div>
                </div>
                <div className="p-8 flex-1 flex flex-col">
                  <p className="text-sm text-[var(--color-gray)] leading-relaxed mb-4 flex-1">{loan.description}</p>
                  <div>
                    <div className="text-xs tracking-wider uppercase text-[var(--color-gray-light)] mb-0.5">Max Amount</div>
                    <div className="text-sm font-semibold">{loan.maxAmount}</div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-[var(--color-charcoal)]/5">
                    <div className="text-xs tracking-wider uppercase text-[var(--color-gray-light)] mb-0.5">Best For</div>
                    <div className="text-sm font-medium text-[var(--color-teal)]">{loan.ideal}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-24 lg:py-32 bg-[var(--color-cream-dark)]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl lg:text-5xl font-light tracking-tight mb-6">
              Total Project <span className="italic text-[var(--color-teal)]">Budgets</span>
            </h2>
            <p className="text-base text-[var(--color-gray)] leading-relaxed max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {budgetExamples.map((b, i) => (
              <div key={b.label} className={`bg-white border p-8 lg:p-10 ${i === 1 ? "border-[var(--color-teal)]/30 ring-1 ring-[var(--color-teal)]/10" : "border-[var(--color-charcoal)]/5"}`}>
                {i === 1 && <span className="inline-block bg-[var(--color-teal)] text-white text-[10px] font-bold tracking-widest uppercase px-3 py-1 mb-4">Most Popular</span>}
                ...
                <div className="space-y-3 mb-6">
                  {[["Home", b.home], ["Delivery & Setup", b.delivery], ["Foundation", b.foundation], ["Utility Connections", b.utilities]].map(([label, value]) => (
                    <div key={label} className="flex justify-between text-sm">
                      <span className="text-[var(--color-gray)]">{label}</span>
                ...
                <div className="pt-4 border-t border-[var(--color-charcoal)]/10 flex justify-between items-end">
                  <div>
                    <div className="text-xs uppercase tracking-wider text-[var(--color-gray)]">Total Budget</div>
                    <div className="font-serif text-2xl font-semibold text-[var(--color-teal)]">{b.total}</div>
                  </div>
                </div>
                <p className="text-xs text-[var(--color-gray)] mt-4 italic">{b.note}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl lg:text-5xl font-light tracking-tight">
              How It <span className="italic text-[var(--color-teal)]">Works</span>
            </h2>
          </div>
            ...
            {[
              ...
              <div key={item.step} className="flex gap-8">
                <div className="flex-shrink-0">
                  <span className="font-serif text-4xl font-light text-[var(--color-teal)]/30">{item.step}</span>
                </div>
                <div>
      ...
      <section className="py-24 lg:py-32 bg-[var(--color-charcoal)] grain-overlay relative text-white">
        <div className="relative max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <h2 className="font-serif text-4xl lg:text-5xl font-light tracking-tight mb-6">
            Ready to Get <span className="italic text-[var(--color-teal-light)]">Pre-Qualified?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="tel:+12603081457" className="btn-primary inline-flex items-center justify-center gap-2 bg-[var(--color-teal)] text-white px-8 py-4 text-sm font-bold tracking-widest uppercase hover:bg-[var(--color-teal-dark)] transition-colors duration-300">

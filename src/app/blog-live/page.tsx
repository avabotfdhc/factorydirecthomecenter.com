...
export const metadata = {
  ...
  description:
    "Manufactured & mobile home news, tips, and guides — rendered in the new design from the admin CMS.",
  robots: { index: false, follow: false },
};

export const dynamic = "force-dynamic";

export default async function BlogLivePage() {
  ...
  return (
    <main className="bg-[var(--color-cream)] text-[var(--color-charcoal)]">
      <section className="bg-[var(--color-charcoal)] text-[var(--color-cream)] py-20 lg:py-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <p className="text-lg text-white/60 max-w-2xl leading-relaxed">
            {posts.length} article{posts.length === 1 ? "" : "s"}, live from your admin CMS — in the new design.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-24">
        {posts.length === 0 ? (
          <p className="text-center text-[var(--color-gray)]">No posts returned from the API right now.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map((p) => (
              <Link
                key={p.slug}
                href={`/blog-live/${p.slug}`}
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  {p.date && (
                    <span className="text-xs tracking-[0.15em] uppercase text-[var(--color-lime-dark)] mb-2">
                      {p.date}
                    </span>
                  )}
